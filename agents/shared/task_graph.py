...
class Task:
    ...
    def to_dict(self) -> dict:
        return {"id": self.id, "title": self.title, "body": self.body,
                "issue_number": self.issue_number, "issue_url": self.issue_url,
                "pr_number": self.pr_number, "state": self.state.value,
                "dependencies": self.dependencies, "assigned_to": self.assigned_to,
...
class TaskGraph:
    ...
    def from_task_list(cls, tasks: list[dict]) -> TaskGraph:
        graph = cls()
        for tdata in tasks:
            task = Task(id=tdata["id"], title=tdata["title"],
                        body=tdata.get("body", ""),
    ...
    def __repr__(self) -> str:
        p = self.progress
        return f"TaskGraph(tasks={p['total']}, completed={p['completed']}, percent={p['percent']:.0f}%)"