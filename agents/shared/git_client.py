...
class GitClient:
    ...
    def list_branches(self, remote: bool = False, pattern: Optional[str] = None) -> list[BranchInfo]:
        ...
        for line in result.stdout.strip().split("\n"):
            ...
            branches.append(BranchInfo(
                name=name, is_current=False, is_remote=is_remote, upstream=upstream,
            ))
        return branches
    ...
    def validate_branch_name(name: str) -> bool:
        import re
        return bool(re.match(r"^[a-z]+-\d+-[a-f0-9]{8}([a-f0-9]{4})?$", name))
    ...
    def status(self) -> list[StatusEntry]:
        ...
        for line in result.stdout.strip().split("\n"):
            ...
            entries.append(StatusEntry(
                path=path, staged_status=status_code[0], worktree_status=status_code[1],
            ))
        return entries
    ...
    def log(self, count: int = 10, branch: Optional[str] = None) -> list[CommitInfo]:
        ...
        args = ["log", f"--pretty=format:{fmt}", "-n", str(count)]
        if branch:
            args.append(branch)
        result = self._run(args)
        commits = []
        for block in result.stdout.strip().split("---"):
            ...
            if len(lines) >= 4:
                commits.append(CommitInfo(
                    hash=lines[0], short_hash=lines[0][:8],
                    subject=lines[1], author=lines[2], date=lines[3],
                ))
        return commits

    def sync_fork(self, upstream_remote: str = "upstream",
                  origin_remote: str = "origin", branch: str = "main") -> None:
        self.fetch(upstream_remote, branch)
        self.checkout(branch)
        self._run(["merge", f"{upstream_remote}/{branch}"])
    ...
    def diff_files(self, branch: str, base: str = "main") -> list[str]:
        result = self._run(["diff", "--name-only", f"{base}...{branch}"], check=False)
    ...
    def generate_branch_name(issue_number: int, hex_length: int = 12, prefix: str = "issue") -> str:
        ...
        return f"{prefix}-{issue_number}-{random_hex}"
