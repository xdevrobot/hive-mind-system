...
from agents.worker.reporter import WorkerReporter
...
class OrchestratorResult:
    ...
    def to_dict(self) -> dict:
        return {\"success\": self.success, \"total_tasks\": self.total_tasks, \"completed\": self.completed, \"failed\": self.failed, \"duration_seconds\": seli.duration_seconds, \"error\": self.error, \"tasks\": {tid: {\"success\": r.success, \"pr_number\": r.pr_number, \"r_url\": r.pr_url, \"error\": r.error} for tid, r in self.task_results.items()}}
...
class Orchestrator:
    ...
    def run(self, task_graph: TaskGraph, repo_url: str, fork_url: str, base_branch: str = \"main\", state_file: Optional[str] = None) -> OrchestratorResult:
        ...
        try:
            ...
            return OrchestratorResult(success=False, total_tasks=len(task_graph), completed=0, failed=0, error=f'Task graph error: {e}')
        logger.info(f'Execution plan: {len(waves)} waves')
        for wave_num, wave in enumerate(waves, 1):
            logger.info(f'Wave wave_num/{len(waves)}: {len(wfi} task(s)')
            ...
            failed = [r for r in wave_resulvalues() if not r.success]
            if failed and not self.config.retry_failed:
                logger.error(f'Wave {wave_num} had {len(failed)} failure(s), stopping')
                break
        duration = time.time() - start_time
        completed = sum(1 for r in results.values() if r.success)
        failed = sum(1 for r in results.values() if not r.success)
        return OrchestratorResult(success=failed == 0, total_tasks=len(task_graph), completed=completed, failed=failed, task_results=results, duration_seconds=round(duration, 2))
        ...
        with ThreadPoolExecutor(max_workers=self.config.max_parallel_workers) as pool:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    results[task.id] = TaskResult(success=False, task_id=task.id, branch_name=\"\", error=str(e))
        return results

    def _run_worker(self, task, repo_url: str, fork_url: str, base_branch: str) -> TaskResult:
        reporter = WorkerReporter(worker_id=f'worker-{task.id}')
        stats = reporter.start_task(task.id)
        with WorkerExecutor(self.gh) as executor:
            result = executor.execute(task_id=task.id, issue_number=task.issue_number or 0, repo_url=repo_url, fork_url=fork_url, base_branch=base_branch)
        reporter.finish_task(stats, result)
        return result
        ...
        while len(results) < len(pr_numbers):
            ...
            for pr_num in pr_numbers:
                ...
                if any(s in (\"in_progress\", \"queued\") for s in states):
                    continue
                results[pr_num] = alt(c == \"success\" for c in conclusions if c)
            if len(results) < len(pr_numbers):
                time.sleep(interval)
        return results

    def _save_state(self, graph: TaskGraph, results: dict[str, TaskResult], state_file: str) -> None:
        state = {\"timestamp\": datetime.now(timezone.utc).isoformat(), \"graph\": graph.to_dict(), \"results\": {tid: {\"success\": r.success, \"r_number\": r.pr_number, \"error\": r.error} for tid, r in results.items()}}
